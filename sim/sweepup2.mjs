// Fine sweep around sm 0.04-0.12, tm 0.15-0.3, at 3p-6p.
// Usage: npx tsx sim/sweepup2.mjs [games]
...
const COMBOS = []
for (const sm of [0.04, 0.06, 0.08, 0.1]) {
  for (const tm of [0.15, 0.2, 0.25, 0.3]) {
    COMBOS.push([sm, tm])
  }
}
for (const [sm, tm] of COMBOS) {
  ...
  console.log(`sm=${String(sm).padEnd(4)} tm=${String(tm).padEnd(4)} ${rows.join(' ')}`)
}
