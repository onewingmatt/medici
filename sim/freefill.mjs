...
const N = 300
let ffEasy = 0, ffHard = 0, ffEasyVal = 0, ffHardVal = 0, days = 0
for (let s = 1; s <= N; s++) {
  ...
  while (g.phase !== 'game_over' && actions < 5000) {
    ...
    if (last.type === 'free_fill') {
      const v = last.cards.reduce((x, c) => x + c.value, 0)
      if (last.playerId === 'easy') { ffEasy++; ffEasyVal += v } else { ffHard++; ffHardVal += v }
    }
    if (last.type === 'day_end') days++
  }
}
console.log(`free fills: easy ${ffEasy} (val ${ffEasyVal}) hard ${ffHard} (val ${ffHardVal}) | days ${days}`)
